...
stats_by_year = {
    ...
    "2029": {"min": -0.136, "max": 0.867, "mean": 0.593},
}
...
threshold_dict = {
    ...
    "2029": {"low": 0.430, "medium": 0.593, "high": 0.756},
}
    ...
    with col2_peta:
        ...
        if option == "1999" or option == "2009":
            ...
            with st.container(border=True):
                ...
                st.info(
                    """
                    Hasil proyeksi NDVI berbasis machine learning (XGBoost) menggunakan data hasil ekstraksi:
                    - NDVI dan penutup lahan dari citra Landsat 5 TM, Landsat 7 ETM+, dan Landsat 8 OLI Surface Reflectance resolusi 30 meter tahun 1999, 2004, 2009, 2014, 2019, dan 2024 dari USGS.
...
with tab3:
    try:
        ...
        st.info(
            "Memeriksa format file CSV dan nama kolom ('ndviLandsat' dan 'ndviSentinel')"
        )
    ...
    st.badge(
        "**Tabel Perbandingan Sampel Nilai NDVI Landsat 8 dan NDVI Sentinel-2**",
        color="primary",
    )

    col_sampel_perbandingan = st.columns(1)
    with col_sampel_perbandingan[0]:
        csv_path = "csv/ndviSampelValidasi.csv"
        ...
        try:
            ...
            df_top10 = df_top10.rename(
                columns={
                    "ndviLandsat": "NDVI Landsat 8",
                    "ndviSentinel": "NDVI Sentinel-2",
                }
            )
            ...
            def convert_df_to_html_ndvi(input_df):
                ...
                for col in input_df.columns:
                    if "ndviLandsat" in col.lower() or "ndviSentinel" in col.lower():
            ...
            html_table = convert_df_to_html_ndvi(df_top10)
            ...
            st.error(f"Error dalam menampilkan tabel sampel validasi NDVI: {str(e)}")
