...
def add_tiles_to_map(map_obj, year):
    try:
        tiles_url = f"https://pramithadi.github.io/spatify2tiles/ndmi/{year}/{{z}}/{{x}}/{{y}}.png"
...
with tab1:
    ...
    with col2_peta:
        ...
        if option == "1999" or option == "2009":
            ...
            with st.container(border=True):
                ...
                st.info(
                    """
                    Hasil proyeksi NDMI berbasis machine learning (XGBoost) menggunakan data hasil ekstraksi:
                    - NDMI dan penutup lahan dari citra Landsat 5 TM, Landsat 7 ETM+, dan Landsat 8 OLI Surface Reflectance resolusi 30 meter tahun 1999, 2004, 2009, 2014, 2019, dan 2024 dari USGS.
    ...
    with col1_peta:
        ...
        if not tiles_success:
            st.info("⏳ Menampilkan peta tanpa layer NDMI. Silakan *refresh* Spatify!")
